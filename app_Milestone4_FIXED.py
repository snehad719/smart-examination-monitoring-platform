...
# ALERT & EVIDENCE HELPERS
def create_alert_db(connection, session_id, alert_type, severity, message):
    connection.execute("""INSERT INTO alerts (session_id, alert_type, severity, message) VALUES (?, ?, ?, ?)""",
                       (session_id, alert_type, severity, message))

def save_frame_evidence(connection, session_id, frame, evidence_type, description):
    os.makedirs("data/evidence", exist_ok=True)
    filename = f'session_{session_id}_{datetime.now().strftime("%Y%m%d_%H%M%S_%f")}.jpg'
    path = os.path.join("data/evidence", filename)
    if cv2.imwrite(path, frame):
        connection.execute("""INSERT INTO evidence (session_id, evidence_type, file_path, description) VALUES (?, ?, ?, ?)""",
                           (session_id, evidence_type, path, description))
        return path
    return None

def save_text_evidence(connection, session_id, evidence_type, description):
    connection.execute("""INSERT INTO evidence (session_id, evidence_type, file_path, description) VALUES (?, ?, ?, ?)""",
                       (session_id, evidence_type, None, description))
...
def evaluate_browser_suspicion(connection, session_id):
    suspicious_events = []
    tab_count = connection.execute("""SELECT COUNT(*) AS n FROM browser_events WHERE session_id = ? AND event_type = 'TAB_SWITCH'""",(session_id,)).fetchone()["n"]
    focus_count = connection.execute("""SELECT COUNT(*) AS n FROM browser_events WHERE session_id = ? AND event_type = 'FOCUS_LOST'""",(session_id,)).fetchone()["n"]

    if tab_count > TAB_SWITCH_THRESHOLD and not suspicious_event_exists(connection, session_id, "More than 3 tab switches"):
        detail=f"More than 3 tab switches detected (count={tab_count})."
        insert_browser_event(connection,session_id,"SUSPICIOUS_EVENT",detail)
        create_alert_db(connection,session_id,"TAB_SWITCH","HIGH",f"Suspicious tab switching detected. Count: {tab_count}")
        save_text_evidence(connection,session_id,"TAB_SWITCH",detail)
        suspicious_events.append(detail)
        print(f"SUSPICIOUS EVENT | TAB_SWITCH | count={tab_count}")

    if focus_count > FOCUS_LOSS_THRESHOLD and not suspicious_event_exists(connection, session_id, "Excessive focus loss"):
        detail=f"Excessive focus loss detected (count={focus_count})."
        insert_browser_event(connection,session_id,"SUSPICIOUS_EVENT",detail)
        create_alert_db(connection,session_id,"FOCUS_LOST","HIGH",f"Excessive focus loss detected. Count: {focus_count}")
        save_text_evidence(connection,session_id,"FOCUS_LOST",detail)
        suspicious_events.append(detail)
        print(f"SUSPICIOUS EVENT | FOCUS_LOST | count={focus_count}")
...
def log_browser_event():
    ...
    suspicious_events = evaluate_browser_suspicion(
        connection,
        exam_session["id"]
    )
